...
export class DatePicker extends HTMLInputElement {
  ...
  get value() {
    return this.#valueDate.toISOString();
  }
  ...
  constructor() {
    ...
    this.#valueDate = new Date(this.getAttribute("value") ?? new Date());
  ...
  renderCalendarHeader(){
    ...
    this.#currentMonthSelect.value = (this.#valueDate.getMonth() + 1).toString().padStart(2, "0");
    this.#currentYearInput = this.#calendarSection.querySelector("input");
    this.#currentYearInput.value = this.#valueDate.getFullYear().toString();
